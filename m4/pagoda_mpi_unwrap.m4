...
AS_VAR_PUSHDEF([pg_save_comp], [pg_save_[]_AC_CC[]])
AS_VAR_PUSHDEF([pg_orig_comp], [pg_orig_[]_AC_CC[]])
AS_VAR_PUSHDEF([pg_cv_mpi_naked], [pg_cv_mpi[]_AC_LANG_ABBREV[]_naked])
AC_CACHE_CHECK([for base $wrapped compiler], [pg_cv_mpi_naked], [
base="`$wrapped -show 2>/dev/null | sed 's/@<:@ 	@:>@.*@S|@//' | head -1`"
pg_save_comp="$_AC_CC"
_AC_CC="$base"
AC_LINK_IFELSE([AC_LANG_PROGRAM([],[])], [pg_cv_mpi_naked="$base"])
_AC_CC="$pg_save_comp"
versions="--version -v -V -qversion"
found_wrapped_version=0
# Try separating stdout and stderr. Only compare stdout.
AS_IF([test "x$pg_cv_mpi_naked" = x], [
# prepend any CC/CXX/F77 the user may have specified
compilers="$pg_orig_comp $compilers"
...
do
            ...
            AS_IF([test "x$naked_compiler" != "x$wrapped"],
                [AS_IF([$naked_compiler $version 1>naked.txt 2>naked.err],
                    [AS_IF([$inside mpi.txt naked.txt >/dev/null],
                        [pg_cv_mpi_naked=$naked_compiler; break],
    ...
    AS_IF([test "x$pg_cv_mpi_naked" != x], [break])
done
])
...
AS_IF([test "x$pg_cv_mpi_naked" = x],
    ...
    [AS_IF([test "x$pg_orig_comp" != x && test "x$pg_orig_comp" != "x$pg_cv_mpi_naked"],
        [AC_MSG_WARN([unwrapped $wrapped ($pg_cv_mpi_naked) does not match user-specified $pg_orig_comp])])])
AS_VAR_POPDEF([pg_save_comp])
AS_VAR_POPDEF([pg_cv_mpi_naked])
rm -f inside.pl
])dnl
...
pg_mpi_unwrap_push_save_CC="$CC"
pg_mpi_unwrap_push_save_CXX="$CXX"
pg_mpi_unwrap_push_save_F77="$F77"
pg_mpi_unwrap_push_save_FC="$FC"
AS_IF([test "x$pg_cv_mpic_naked"   != x], [ CC="$pg_cv_mpic_naked"])
AS_IF([test "x$pg_cv_mpicxx_naked" != x], [CXX="$pg_cv_mpicxx_naked"])
AS_IF([test "x$pg_cv_mpif77_naked" != x], [F77="$pg_cv_mpif77_naked"])
AS_IF([test "x$pg_cv_mpifc_naked"  != x], [ FC="$pg_cv_mpifc_naked"])
])dnl
...
AC_DEFUN([PAGODA_MPI_UNWRAP_POP], [
 CC="$pg_mpi_unwrap_push_save_CC"
CXX="$pg_mpi_unwrap_push_save_CXX"
F77="$pg_mpi_unwrap_push_save_F77"
 FC="$pg_mpi_unwrap_push_save_FC"
])dnl
