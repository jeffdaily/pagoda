...
int main(int argc, char **argv)
{
    ...
    int g_dst = NGA_Create(MT_INT, 1, &size, "dst", NULL);
    int g_msk = NGA_Create(MT_INT, 1, &size, "msk", NULL);
    int ONE_NEG = -1;
    int icount;
    ...
    if (0 == me) {
        ...
        //int vals[] = {0,0,1,0,1,1,1,0,0,1}; // 10 == size, count = 5
        int vals[] = {1,0,1,0,1,1,1,0,0,1}; // 10 == size, count = 6
        int lo = 0;
        int hi = size-1;
    ...
    //GA_Unpack(g_src, g_dst, g_msk, 0, size-1, &icount);
